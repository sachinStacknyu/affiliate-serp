...
// // // SerpAPIResponse is used for hotel-search mode (returns a list of properties).
...
// // // ToSerpAPIResponse wraps a single property detail into the standard list
// // // response shape so callers always get the same JSON structure.
...
// // // Flex handles fields that may be a native type OR a string in the JSON.
...
// // 	body, _ := json.Marshal(AffiliateLinkRequest{
// // 		Trs: trs, Marker: marker, Shorten: true,
// // 		Links: []Link{{URL: propertyURL}},
// // 	})
// // 	req, err := http.NewRequest("POST", "https://api.travelpayouts.com/links/v1/create", bytes.NewBuffer(body))
// // 	if err != nil {
// // 		return propertyURL
// // 	}
...
// // 	apiURL := fmt.Sprintf("https://serpapi.com/locations.json?q=%s&limit=5", url.QueryEscape(cityName))
...
// // 	// Detail-mode only (mutually exclusive with search filters)
...
// // 		return fetchPropertyDetail(&serpapi.SerpApiClient{}, opts)
// // 	}
// // 	return fetchHotelList(client, opts)
// // }

// // // fetchHotelList — normal search returning multiple properties.
// // func fetchHotelList(client serpapi.NewClient, opts HotelSearchOptions) (*SerpAPIResponse, error) {
...
// // // fetchPropertyDetail — detail mode using property_token.
// // //
// // // Key facts from SerpAPI docs:
// // //   - property_token is required; most other params are ignored.
// // //   - "q" must still be present (known SerpAPI bug, marked wontfix).
// // //   - The response is a SINGLE property at the top level, NOT inside a
// // //     "properties" array — so we use PropertyDetailResponse and convert.
...
// // 	params := cleanParams(map[string]string{
// // 		// "q" must be present even in detail mode (SerpAPI bug, wontfix).
// // 		"q":              "hotel",
...
// // 		return nil, err
// // 	}
// // 	log.Printf("Property-detail raw response: %s", string(raw))

// // 	// SerpAPI returns a single-property object at the top level in detail mode,
// // 	// not a "properties" array. Decode into PropertyDetailResponse then normalise.
// // 	var detail PropertyDetailResponse
// // 	if err := json.Unmarshal(raw, &detail); err != nil {
// // 		return nil, fmt.Errorf("unmarshal property detail: %w", err)
// // 	}
